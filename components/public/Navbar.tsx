...
export default function Navbar() {
  ...
  return (
    <header className='sticky top-0 z-50 bg-[#111111]/95 backdrop-blur-sm border-b border-white/5'>
      <div className='mx-auto flex max-w-7xl items-center justify-between px-6 py-5'>
        ...
        <div className='hidden items-center gap-4 md:flex'>
          <Link
            href='/reservations'
            className='rounded-full bg-orange-500 px-6 py-2.5 text-sm font-semibold text-white shadow-lg shadow-orange-500/20 transition hover:bg-orange-600'
          >
            Book Table
          </Link>
        </div>
          ...
          <div className='flex flex-col gap-2'>
            {links.map((link) => (
              <Link
                ...
                className='rounded-2xl px-4 py-3 text-slate-300 transition hover:bg-white/5'
              >
                {link.label}
              </Link>
            ))}
